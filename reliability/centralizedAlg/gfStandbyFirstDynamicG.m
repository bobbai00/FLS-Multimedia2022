function [reliabilityGroups, verticesHaveNoGroup] = gfStandbyFirstDynamicG(cube, G, vertexList)
    ...
    if size(verticesHaveNoGroup, 2) > 0
        dynamicGroup = reliabilityGroup(cube.identity, G, verticesHaveNoGroup, vertexList);
        dynamicGroup.regardCenterAsStandbyFls(vertexList);
        reliabilityGroups = [reliabilityGroups, dynamicGroup];
        verticesHaveNoGroup = [];
    end
